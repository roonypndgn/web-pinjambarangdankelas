...
<body>
    ...
    <table>
        ...
        <tbody>
            @foreach ($pengembalians as $i => $pengembalian)
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td>{{ $pengembalian->pinjam->user->nama }}</td>
                    <td>{{ $pengembalian->pinjam->barang->merk }}</td>
                    <td>{{ \Carbon\Carbon::parse($pengembalian->pinjam->tgl_pinjam)->format('d-m-Y') }}</td>
    ...
    <div class="footer">
        &copy; {{ date('Y') }}  pinjamFIKOM  - Fakultas Ilmu Komputer Universitas Methodist Indonesia<br>
    </div>
